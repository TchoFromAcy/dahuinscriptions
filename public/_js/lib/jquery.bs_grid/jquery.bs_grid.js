        ...
        init: function(options) {
            ...
            return this.each(function() {
                ...
                for(var i in settings.columns) {
                	...
                	if(!settings.columns[i].type||settings.columns[i].type!=="action") {
 ...
 */
        ...
        updateRow:function(id){
        	...
        	var data=$(this).data('data')[id ], s = methods.getAllOptions.call(this),row=$('#tbl_'+$(this).attr('id')+'_tr_'+id)
        	        	
        	var colIndex=0;
        	
        	$.each(s.columns, function(index, column){
        		if(column.type&&column.type=='action'||!column_is_visible(column)) return;
        		var value=data[column.field],html=(column.renderer&&typeof column.renderer=="function"?column.renderer(value, data):value);
        		var td=row.find('td[data-colIndex='+index+']');
        		$(td).html(html);
        ...
        getCellValue:function(id, colIndex, raw){
               	
        	if(raw==undefined) raw=false;
        	
        	var data=$(this).data('data')[id], cols=methods.getOption.call(this,'columns'),column=cols[colIndex],value=data[column.field];
        	return (column.renderer&&typeof column.renderer=="function"&&!raw?column.renderer(value, data):value);
                ...
                success: function(data) {
                    ...
                    for(i in s.columns) {
                        if(column_is_visible(s.columns[i])) {
                            ...
                            tbl_html += '<th class="' + s.commonThClass + '" data-colIndex="'+i+'">' + s.columns[i].header + sortingIndicator + '</th>';
                            
                           // tbl_html.data('colIndex',i);
                        }
                    }
                    tbl_html += '</tr>';
                    tbl_html += '</thead>';
                    ...
                    for(row in page_data) {
                        ...
                        for(i in s.columns) {
                            if(column_is_visible(s.columns[i])) {
                            	...
                            	var html=(s.columns[i].renderer&&typeof s.columns[i].renderer=="function"?s.columns[i].renderer(page_data[row][s.columns[i].field], page_data[row]):page_data[row][s.columns[i].field]);
                           
                            	recordSet[page_data[row][row_primary_key]]=page_data[row];
                            	
                                tbl_html += '<td data-colIndex="'+i+'">' +  html+ '</td>';
                            }
                        }
