...
define('PRIX_JOUEUR', 60);
define('PRIX_NONJOUEUR', 45);
define('PRIX_ENFANT', 30);

define('BTC_PHOTO_LINK', 'https://photos.google.com/share/AF1QipMdAZFrq9Qx3AOjAG0eY39Qixz3orojlcBYbQ5jmtVv2ZU6_dntl2C175_Sh110zQ?key=ekJWazMtR0VJNnFBeHNBcHlldEZJZDY5RUx6bk53');
...
class IndexController extends Zend_Controller_Action {
	...
	public function init() {
		...
		$this->isDev=false;//
      //  if($_SERVER['REMOTE_ADDR']=="77.150.250.93") die($this->_request->getActionName());
	...
	public function preDispatch() {
		$this->context=preg_match('/bornes/',$_SERVER['HTTP_HOST'])||preg_match('/localhost/',$_SERVER['HTTP_HOST'])?'tournoi':'inscription';
		if($this->context=="tournoi"&&$this->_request->getActionName()!=="tournoi"&&$this->_request->getActionName()!=="ajax"&&$this->_request->getActionName()!=="updater"&&$this->_request->getActionName()!=="Admin"&&$this->_request->getActionName()!=="getPhoto"&&$this->_request->getActionName()!=="Login"&&$this->_request->getActionName()!=="sendpayement"):
		$this->_helper->layout->setLayout('tournoi'.($this->isMobile?"-mobile":""));
		
		if($this->isDev) $this->_helper->layout->setLayout('tournoiconfirm');
		
		
		$this->_forward('tournoi');
	...
	public function indexAction() {
		...
		if ($this->getRequest ()->authKey) :
			
			$mapper = new Default_Model_InscriptionMapper ();
			$user = $mapper->getInscriptionFromHash ( $this->getRequest ()->authKey );
			
			if ($user) :
				
				$this->view->userData = ( object ) $user->dataToArray ();
				$this->view->hash = $this->getRequest ()->authKey;
				$this->view->arrayFiles = $this->getFormDoc ( $this->getRequest ()->authKey );
				if ($user->__get ( "saison" ) !== $this->currentSeason) :
					unset ( $this->view->arrayFiles ['certif'] );
					unset ( $this->view->userData->certif );
					...
					$this->view->inscriptionState = "renew";
				 elseif ($user->__get ( 'statut' ) == 1) :
					
					$this->view->statusClosed = true;
	...
	public function finalize() {
		...
		$content .= "<li>Le montant de ta cotisation pour la licence " . ($inscriptionModel->__get ( 'licence' ) == 2 ? " comp&eacute;tition" : 'loisir') . " soit " . $prix . " &euro; par ch&egrave;que (&agrave; l'ordre de l'association Dahultimate) ou par virement (<a href=\"http://{$_SERVER['HTTP_HOST']}/medias/rib.pdf\">Le rib du club est ici</a>)</li>";
	...
	public function tournoiAction(){
		...
		$this->_helper->viewRenderer('tournoi-mobile');
                 ...
                 if($_SERVER['REQUEST_URI']==='/exporttournoi') $this->exporttournoiAction();
		
               if($_SERVER['REMOTE_ADDR']==="37.71.122.170") :
		...
		if($this->isDev)$this->_helper->viewRenderer("maintenance");
             ...
             //   $this->view->closed = $_SERVER['REMOTE_ADDR']==="37.71.122.170"?false:true;
		...
		$this->view->max = preg_match('/localhost/',$_SERVER['HTTP_HOST'])?40:$this->maxInscription;
	}
	...
	public function subscribeTournament(){
        ...
        $waitinglist=$post['waitinglist'];

		$post=(object)$post;
		$ln = $post->ln;
		$token=$post->token;
      //  return ['success'=>false, 'post'=>$post->waitinglist];
...
<br />73410 LA BIOLLE";
		...
		$arrayLang=(object)["fr"=>
		(object)[
		    "WAINTING_LIST"=>"Tu es sur liste d'attente, nous reviendrons vers toi dès qu'une place se libère",
		...
		"MSG_PAYMENT"=>"Ce serait cool que tu puisses payer rapidement (avant le 30 juin si possible) la somme de %s : <ul>".
		"<li>par ch&egrave;que, &agrave; l'ordre de l'association Dahultimate, &agrave; envoyer &agrave; :<br />$addrFanch</li>
		<li>par virement (<a href=\"http://{$_SERVER['HTTP_HOST']}/medias/rib.pdf\" style=\"color:black\">Le rib du club est ici</a>)</li></ul>",
		"MSG_NOTICE"=>"Tu recevras tr&egrave;s rapidement les derni&egrave;res nouvelles.",
				...
				"MSG_SAISPAS"=>"comment &ccedil;a, tu ne sais pas si tu vas venir au Bornes to Catch ?.<br />%s<br />Allez, confirme nous vite ton inscription !",
						...
						"uk"=>
						(object)[
                            "WAINTING_LIST"=>"Yoou are on the waiting list, we'll be back to you when you are in",
						...
						"MSG_PAYMENT"=>"It would be nice if you could paye the fee of %s : <ul><li>by bank check (payable to Association Dahultimate) to the following address : <br />$addrFanch</li>
								<li>or by bank transfer(<a href=\"http://{$_SERVER['HTTP_HOST']}/medias/rib.pdf\" style=\"color:black\">Our RIB is here</a>)</li></ul>",
        ...
        if(preg_match('/localhost/',$_SERVER['HTTP_HOST'])) $ok=false;
			...
			return array (
							...
							"error" => $tradf->CAPTCHA_ERROR
										...
										// test age
												$date = new Zend_Date ( $post ->dob, $this->isMobile?'dd/MM/yyyy':'dd/MM/yyyy' );
                                                                                               
												$today = new Zend_Date('2018-08-18');
		...
		if((int)$post->waitinglist === 1)$post->confirm=4;
		...
		$playlistURL="http://www.deezer.com/playlist/4472473084?utm_source=deezer&utm_content=playlist-4472473084&utm_term=14147506_1526321087&utm_medium=web";
           ...
           //  echo $recap;
			...
			if((int)$joueur->__get('confirm')!==4)	$recap.="<br />".sprintf($tradf->MSG_MODIF_LINK, "http://".$_SERVER['HTTP_HOST']."/confirm?token=$token&ln=$ln", "http://".$_SERVER['HTTP_HOST']."/confirm?token=$token&ln=$ln");;
		...
		$mail->sendMail($joueur->__get('email'), $emailMsg, $tradf->EMAIL_SUBJECT, ["address"=>"tournoi@dahultimate.com","name"=>"Bornes to be 3"]);
		$mail->sendMail("chau@plusdeclic.com", $emailMsg, "copie:".$tradf->EMAIL_SUBJECT, ["address"=>"tournoi@dahultimate.com","name"=>"Bornes to be 3"]);
			...
			break;
                            ...
                            case 4 :
                                $subject=utf8_decode($joueur->__get('prenom'))." ".utf8_decode($joueur->__get('nom'))." est sur liste d'attente !";

                                $recap.=" est sur liste d'attente.";
                                break;
			...
			$mail->sendMail($this->isDev?"chau@plusdeclic.com":"tournoi@dahultimate.com", $emailMsg, $subject);
				...
				return ["success"=>true, "msg"=>sprintf($tradf->CONFIRM,($joueur->__get('prenom'))),'debug'=>$emailMsg];
	...
	public function checkToken($token){
		...
		$mapper=new Default_Model_TournoiMapper();
		$query=$mapper->fetchAll("confirm=1 and (payed is null or payed=0) and update_date like '%2018%'");
...
<br />73410 LA BIOLLE";
		...
		$message="<p style=\"$style\">Salut ".utf8_decode($joueur->prenom).",<br />apparemment nous n'avons toujours pas re&ccedil;u ton paiement pour le Bornes 2 be 3 .<br />";
		...
		$message.=sprintf("Ce serait cool que tu puisses payer rapidement (avant le 17 juillet si possible) la somme de %s &euro;: </p><ul style=\"$style\">".
		"<li style=\"$style\">par ch&egrave;que, &agrave; l'ordre de l'association Dahultimate, &agrave; envoyer &agrave; :<br />$addrFanch</li>
		<li style=\"$style\">par virement (<a href=\"http://{$_SERVER['HTTP_HOST']}/medias/rib.pdf\" style=\"color:black\">Le rib du club est ici</a>)</li></ul>",$prix);
        ...
        public function exporttournoiAction(){
		...
		$fp = fopen ( APPLICATION_PATH . '/export/exportB2C.csv', 'w+' );
		
                //fputcsv ( $fp, $headers, ";" );
                
                $participants = $mapper->fetchAll("confirm=1");
                
		foreach($participants as $joueur):
            
                    $joueur=array_map('utf8_decode',$joueur);
                    
                    $postes=explode(',',$joueur['poste']);
                     $array=[
                        "nom"=>$joueur["nom"],
                    ...
                    foreach($postes as $poste):
                        $array['handler']=(int) $poste==1?1:0;
                    $array['middle']=(int) $poste==2?1:0;
                    $array['deep']=(int) $poste==3?1:0;
                        $array['technique']=$joueur["technique"];
                        $array['physique']=$joueur["physique"];
                    endforeach;
