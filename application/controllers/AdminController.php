...
class AdminController extends Zend_Controller_Action
{
    ...
    public function indexAction()
    {
        ...
        if ($this->context == "tournoi"):
            $this->_helper->layout->setLayout('tournoi-admin');
...
<br />73410 LA BIOLLE";
        ...
        if (!empty($token)):
            ...
            if (!$check) {
                return ["success" => false, "error" => $tradf->TOKEN_ERROR];
            };
    ...
    public function exportteamsAction()
    {
        ...
        if (isset($_GET['pdf'])) return $this->_exportTeamsPdf();

        $this->_helper->layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
        $teamsMapper = new Default_Model_B2cTeamsMapper();
        $teams = $teamsMapper->fetchAll();
        ...
        $cols = ['nom', 'prenom', 'club', 'dob', 'poste', 'sexe', 'physique', 'technique'];
        ...
        foreach ($teams as $key => $team) {
            ...
            if ($key > 0) {
                ...
                $myWorkSheet = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, $team['nom']);
...
// Attach the "My Data" worksheet as the first worksheet in the Spreadsheet object
            ...
            $sheet = $spreadsheet->getActiveSheet();
            $sheet->setTitle($team['nom']);

            $players = $playersMapper->fetchAll('id_team=' . $team['id_team']);

            $players = array_map(function ($player) use ($cols) {

                $player['nom'] = strtoupper($player['nom']);

                foreach ($player as $key => $value) {

                    if (!in_array($key, $cols)) unset($player[$key]);
            ...
            $sheet->fromArray(['Nom', 'Prénom', 'Date de naissance', 'Sexe', 'Club', 'Poste(s)', 'Technique', 'Physique'], null, 'A1');
            foreach ($players as $player) {

                $sheet->fromArray($player, null, 'A' . $row);
                $row++;
        ...
        $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
        $writer->setOffice2003Compatibility(true);
        $writer->save(APPLICATION_PATH . '/export/export_teams.xlsx');
        ...
        $zip->addFile(APPLICATION_PATH . '/export/export_teams.xlsx', 'equipes.xlsx');

        $zip->close();

        header('Content-Type: application/zip');
        header('Content-Length: ' . filesize($file));
        header('Content-Disposition: attachment; filename="export_teams.zip"');
        readfile($file);
        unlink($file);
    ...
    function _exportTeamsPdf()
    {
        ...
        $zip = new ZipArchive();
        $zip->open($file, ZipArchive::OVERWRITE);
        foreach ($teamMapper->fetchAll() as $team) {



            $content = file_get_contents('http://' . $_SERVER['HTTP_HOST'] . '/equipe/' . $team['id_team']);
            ...
            $config = ([
                'fontDir' => array_merge($fontDirs, [
                    APPLICATION_PATH . '/fonts',
                ]),
                'fontdata' => $fontData + [
                        'Yikes' => [
                            'R' => 'yikes__-webfont.ttf',
            ...
            $mpdf->Output(APPLICATION_PATH . '/export/equipe_' . $team['id_team'].'.pdf',\Mpdf\Output\Destination::FILE);

            $zip->addFile(APPLICATION_PATH . '/export/equipe_' . $team['id_team'].'.pdf', $team['nom'] . '.pdf');
        ...
        header('Content-Type: application/zip');
        header('Content-Length: ' . filesize($file));
        header('Content-Disposition: attachment; filename="fiches_equipes.zip"');
        readfile($file);
        unlink($file);
    ...
    public function exportmatchsAction(){
        ...
        $content = file_get_contents('http://' . $_SERVER['HTTP_HOST'] . '/matchsList');
        $mpdf = new \Mpdf\Mpdf();
        $mpdf->WriteHTML($content);
        $mpdf->Output();
        die();

        $mpdf->Output(APPLICATION_PATH . '/export/matchs.pdf',\Mpdf\Output\Destination::FILE);

        $zip->addFile(APPLICATION_PATH . '/export/matchs.pdf', 'matchs.pdf');
...
header('Content-Type: application/zip');
header('Content-Length: ' . filesize($file));
header('Content-Disposition: attachment; filename="matchs.zip"');
readfile($file);
unlink($file);
